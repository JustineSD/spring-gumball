...
public class GumballMachineController {
    ...
    @Autowired
    private GumballRepository gumballRepository;

    @GetMapping
    public String getAction(@ModelAttribute("command") GumballCommand command, Model model, HttpSession session) {
        GumballModel gModel = new GumballModel();
        Optional<GumballModel> gumballs = gumballRepository.findById("2134998871109");

        gModel = gumballs.get();

        model.addAttribute("gumball", gModel);
        
        GumballMachine gm = new GumballMachine();
        String message = gm.toString(gModel.getModelNumber(), gModel.getSerialNumber());
    ...
    public String postAction(@Valid @ModelAttribute("command") GumballCommand command,
        ...
        long prevTimestamp = Long.parseLong(timestamp);

        if (((currentTimestamp - prevTimestamp) / 1000) > 1000) {
        ...
        Optional<GumballModel> object = gumballRepository.findById("2134998871109");
        GumballModel gModel1 = new GumballModel();
        gModel1 = object.get(); 
        ...
        if (action.equals("Turn Crank")) {
            ...
            int prevCounter = gModel1.getCountGumballs();
            gModel1.setCountGumballs(prevCounter - 1);
            gumballRepository.save(gModel1);
        }

        String message = gm.toString(gModel1.getModelNumber(), gModel1.getSerialNumber());
